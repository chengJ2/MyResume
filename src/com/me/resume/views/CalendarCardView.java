...
public class CalendarCardView extends View{
    ...
    private Paint mText2Paint; // 绘制文本的画笔
    private Paint mElemetPaint;// 绘制当天有事件时的标记
    ...
    public CalendarCardView(Context context, OnCellClickListener listener) {
        super(context);
    ...
    private void init(Context context) {
        ...
        mElemetPaint= new Paint(Paint.ANTI_ALIAS_FLAG);
        
        mText2Paint.setColor(Color.parseColor("#BFBFBF"));
        mCirclePaint.setStyle(Paint.Style.FILL);
        mCirclePaint.setColor(Color.parseColor("#F24949"));// 红色圆形
        mElemetPaint.setStyle(Paint.Style.FILL);
        mElemetPaint.setColor(Color.parseColor("#00CC00"));
    ...
    private void fillDate() {
        ...
        int monthDay = TimeUtils.getCurrentMonthDay(); // 今天
        int lastMonthDays = TimeUtils.getMonthDays(mShowDate.year,
                mShowDate.month - 1); // 上个月的天数
        int currentMonthDays = TimeUtils.getMonthDays(mShowDate.year,
                mShowDate.month); // 当前月的天数
        int firstDayWeek = TimeUtils.getWeekDayFromDate(mShowDate.year,
                mShowDate.month);
        boolean isCurrentMonth = false;
        if (TimeUtils.isCurrentMonth(mShowDate)) {
            isCurrentMonth = true;
        }
        int day = 0;
...
//                MyLog.d("year:"+year + "  month:"+month + "  day:"+day);
                /*if(year== 2015 && month==8 && day == 9){
                	invalidate();
                	rows[j].cells[i] = new Cell(CustomDate.modifiDayForObject(mShowDate, day), State.ELEMENT_DAY, i, j);
                }*/
            }
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mText2Paint.setTextSize(mCellSpace / 5);
    }
    ...
    class Cell {
        ...
        public void drawSelf(Canvas canvas) {
            ...
            case TODAY: // 今天
                ...
                mText2Paint.setColor(Color.parseColor("#00ffffff"));
            ...
            case NEXT_MONTH_DAY: // 下一个月
                mTextPaint.setColor(Color.parseColor("#fffffe"));
                mText2Paint.setColor(Color.parseColor("#fffffe"));
                break;
            case UNREACH_DAY: // 还未到的天
                mTextPaint.setColor(Color.GRAY);
                mText2Paint.setColor(Color.GRAY);
                break;
            case ELEMENT_DAY:
            	canvas.drawCircle((float) (mCellSpace * (i + 0.5)), 
            			(float) ((j) * mCellSpace), 20, mElemetPaint);
            	break;
            default:
                break;
            }
            ...
            long[] nongli = TimeUtils.calElement(date.year, date.month, date.day);
            canvas.drawText(TimeUtils.getChinaDayString((int)nongli[2]),
                    (float) ((i + 0.5) * mCellSpace - mTextPaint
                            .measureText(content) / 2), (float) ((j + 0.7)
                            * mCellSpace - mTextPaint
                            .measureText(content, 0, 1) / 2)+50, mText2Paint);
        }
    }
     ...
     */
    enum State {
        TODAY,CURRENT_MONTH_DAY, PAST_MONTH_DAY, NEXT_MONTH_DAY, UNREACH_DAY,ELEMENT_DAY;
    }
